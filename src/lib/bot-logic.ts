...
function getAuthUrl() {
    return `https://api-v2.upstox.com/login/authorization/dialog?response_type=code&client_id=${config.UPSTOX_API_KEY}&redirect_uri=${config.UPSTOX_REDIRECT_URI}`;
}
...
class UpstoxAPI {
    ...
    static async isTokenValid(accessToken: string | null | undefined): Promise<boolean> {
        if (!accessToken) return false;
        const headers = this.getHeaders(accessToken);
        const url = "https://api-v2.upstox.com/v2/user/profile"; // Lightweight endpoint
        try {
...
export async function getBotResponse(message: string, token: string | null | undefined, portfolio: Portfolio): Promise<BotResponsePayload> {
    ...
    // Check if the input is a URL containing a code
    if (command.includes('?code=')) {
        try {
            const url = new URL(command);
            authCode = url.searchParams.get('code') || '';
        } catch (e) {
            // Not a valid URL, proceed as normal
        }
    }
    
    // If we extracted a code, use it immediately
    if (authCode) {
        try {
    ...
    // Proactive token check for sensitive commands
    const commandsNeedingToken = ['start', '/portfolio', '/close', '/paper', 'exp:'];
    if (commandsNeedingToken.some(cmd => mainCommand.startsWith(cmd))) {
         const tokenIsValid = await UpstoxAPI.isTokenValid(token);
         if (!tokenIsValid) {
              return { 
                type: 'error', 
                message: `Your Upstox access token has expired or is invalid. Please re-authorize.`, 
                authUrl: getAuthUrl(),
                portfolio
            };
         }
    ...
    switch (mainCommand) {
        case 'start':
            try {
                ...
                if (e.message.includes("Access Token is not configured") || e.message.includes("invalid or has expired")){
                    return { type: 'error', message: e.message, authUrl: getAuthUrl(), portfolio };
                }
        ...
        case 'help':
            const helpText = `**VizBot NIFTY Options Analysis**
...
2. After logging in, copy the full URL from the new browser tab.
3. Paste the URL into the chat. The bot will automatically find the code.
