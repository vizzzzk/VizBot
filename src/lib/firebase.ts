...
for (const [key, value] of Object.entries(firebaseConfig)) {
  if (!value) {
    throw new Error(`[Firebase config] Missing environment variable ${key}. Please check your .env file and restart the server.`);
  }
}

if (!String(firebaseConfig.apiKey).startsWith("AIza")) {
  throw new Error("[Firebase config] apiKey does not look valid (should start with 'AIza'). Please check your .env file.");
}
